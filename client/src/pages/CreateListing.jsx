...
export default function CreateListing() {
  ...
  if (files.length > 0 && files.length <= 6) {
    ...
    try {
      ...
      const validPaths = uploadedImagePaths.filter((path) => path); // remove nulls

      if (validPaths.length > 0) {
        setFormData((prevFormData) => ({
          ...prevFormData,
          imageUrls: [...prevFormData.imageUrls, ...validPaths],
        }));
      }

      console.log('Uploaded:', validPaths);
    } catch (error) {
      ...
      <form onSubmit={handleSubmit} className='flex flex-col sm:flex-row gap-4'>
        <div className='flex flex-col gap-4 flex-1 font-semibold'>
          ...
          <div className='flex flex-wrap gap-6'>
            ...
            <div className='flex items-center gap-2'>
              <input type="number" id='regularPrice' min='50' max='1000' required  className='p-3 border border-gray-300 rounded-lg'
        ...
        <div className='flex flex-col flex-1 gap-4'>
          ...
          <div className='flex gap-4'>
            ...
            <button type='button' disabled={loading || uploading} onClick={handleImageSubmit} className='p-3 text-green-700 border border-green-700 rounded uppercase hover:shadow-lg disabled:opacity-80'>
